...
interface SidebarItemProps {
    item: {
        ...
        href: string; // Opcional, pode ser '#' se for apenas um agrupador
        children?: SubItem[];
    };
    isOpen: boolean; 
}
...
const SidebarItem: React.FC<SidebarItemProps> = ({ item, isOpen }) => {
    ...
    // 1. REINTRODUZINDO o useState para o toggle manual
    const [isExpanded, setIsExpanded] = useState(false);
    
    // Verifica se a rota atual é um sublink deste item
    const isRouteActive = item.children?.some(child => pathname.startsWith(child.href));
    
    // 2. Usando useEffect para FORÇAR a expansão se a rota estiver ativa
    // Isso garante a persistência do estado ao navegar entre as rotas filhas.
    useEffect(() => {
        if (isRouteActive) {
            setIsExpanded(true);
        }
        // NOTA: Não forçamos para false aqui. O usuário deve fechar manualmente
        // ou a re-renderização inicial (se a rota for outra) usará o useState(false).
    }, [isRouteActive]); 

    // O item está ativo se o link pai for a rota ou se qualquer sublink estiver ativo
    const isLinkActive = pathname === item.href || isRouteActive;
    ...
    // Se tiver filhos (Nested Links)
    if (item.children) {
        return (
            <div className="w-full">
                {/* 3. Reativando o onClick para o toggle manual */}
                <button
                    onClick={() => setIsExpanded(!isExpanded)}
                    className={`${baseClasses} ${isLinkActive ? activeClasses : inactiveClasses} w-full`}
                    title={titleText}
                >
                    <div className="flex items-center">
                        <ItemIcon size={20} className={isOpen ? "mr-3" : "mx-auto"} /> 
                    ...
                    {/* Exibe a seta apenas quando a sidebar está aberta */}
                    {isOpen && <ChevronIcon size={16} className="ml-2" />}
                </button>
    ...
    // Se não tiver filhos (Link Simples)
    return (
        <Link 
            href={item.href} 
            className={`${baseClasses} ${isLinkActive ? activeClasses : inactiveClasses} w-full`}
