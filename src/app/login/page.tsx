...
export default function Login() {
    ...
    // Lógica do Vanta.js (mantida)
    useEffect(() => {
        if (!vantaEffect && vantaRef.current) {
            // Garante que o Vanta.js se inicialize corretamente
            import("vanta/dist/vanta.fog.min").then((VANTA) => {
                const effect = VANTA.default({
                    el: vantaRef.current,
                    THREE: THREE,
    ...
    return (
        <div
            ref={vantaRef}
            // 🛑 AJUSTE 1: Garantir que o formulário é centrado em todas as telas por padrão, 
            // e só é empurrado para a esquerda em 'md' e 'lg' (mantendo sua lógica).
            // px-4 garante espaçamento em mobile.
            className="relative min-h-screen flex items-center justify-center text-white p-4"
            style={{overflow: "hidden"}}
        >
            {/* 🛑 AJUSTE 2: Garantir que o overlay cobre 100% da tela */}
            <div 
                className="absolute inset-0 bg-linear-to-br from-black via-black/80 to-neutral-950/20" 
            />
            ...
            <div
                className="
                    ...
                    w-full 
                    max-w-md 
                    // 🛑 AJUSTE 3: Adicionar a lógica de alinhamento ao div que contém o Card.
                    md:mr-auto md:ml-20 lg:ml-32 
                "
            >
                {/* 🛑 AJUSTE 4: Adicionar a transição de cor do texto para o foreground */}
                <Card
                    className="bg-[#161616]/80 backdrop-blur-sm border-[#161616]/50 shadow-2xl transition-colors"
                >
                    <CardHeader className="space-y-4 text-center">
                        <div className="mx-auto w-16 h-16 bg-[#58a547]/10 rounded-2xl flex items-center justify-center">
                            <Building2 className="w-8 h-8 text-[#58a547]"/>
